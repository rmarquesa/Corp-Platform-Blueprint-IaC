  ...
  provisioner "local-exec" {
    command = <<-EOT
      ssh -o StrictHostKeyChecking=no root@${var.proxmox_host} '
        set -euo pipefail
        conf="/etc/pve/lxc/${var.vm_id}.conf"
        ...
        grep -qxF "lxc.cgroup2.devices.allow: c 10:200 rwm" "$conf" || \
          echo "lxc.cgroup2.devices.allow: c 10:200 rwm" >> "$conf"
        grep -qxF "lxc.mount.entry: /dev/net/tun dev/net/tun none bind,create=file" "$conf" || \
